...
In the last decades, reinforcement learning paradigm, despite being actually not a new concept, has gained decent amount of popularity. Starting from middle 80's and ever since then, it is applied in many fields *ranging from X Y Z* due to its generic formulation to problems. *Give brief RL formulation*. However, the main reason to get such attention is due to recently breakthrough success in the game context by Google's DeepMind.
\begin{itemize}
