...
Semi-supervised learning in some sense, self-training, and stochastic environment of the backgammon game.

Automatic feature extraction or an agent cannot sense the entire state of the environment that might not converge/emerge on something leading to poor agent performance.

-- Paragraph 3 --

\begin{itemize}
    \item Large amounts of hand labeled data
    \item Scalar reward, frequently sparse, noisy and delayed
    \item Assumption in deep learning applications concerning data samples are independent, in RL they are highly correlated.
    \item Deep learning assumes fixed underlying distribution, in RL data distribution changes as the algorithm learns
\end{itemize}
...
\colorbox{red}{HIGHLY INCOMPLETE AND MEANINGLESS DRAFT}
...
\begin{itemize}
    ...
    \item Hard experience about writing and investigating RL papers, categorization. Theoretical framework could be useful to provide a structured way to describe RL articles as surveys. Similar frameworks exist in the view of computational creativity world. This would provide more structural way for researches to present their case studies, but also to describe the candidate systems regardless of what they use. A theoretical framework, such as that presented here may be useful for some issues, but it may also be useful in giving generalized descriptions of behaviors just as in computationally creative agents. 
\end{itemize}
