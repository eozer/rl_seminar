...
In the literature, there is no clear-cut definition to categorize reinforcement learning applications in the gaming domain under any framework. So, we categorize the systems based on the abstraction approaches, as we have already defined in the earlier sections (Section 2.2). Table \ref{table:comparison} demonstrates a comparison matrix among four reviewed reinforcement learning articles based on the criteria.

Firstly, previous articles \cite{tesauro1995temporal, liao2012cs229} utilize low-level abstraction in two different game genres board game and platformer game. Usually, board games need game specific information such as position or number of the piece.
\newpage
 \begin{table}[h]
 	\begin{center}
 		\hspace*{-1.25cm}
 		\includegraphics[width=1.1\textwidth]{img/comparison.png}
 		\caption{The comparison matrix presents the differences among four reviewed reinforcement learning articles in the gaming context based on the evaluation criteria.}
...
Express what we did in this paper.

Go all over again the reviewed articles, in a brief summary. Give your opinions about those papers.

Sandbox learning. Context-free -> no specific information, model-free no need for transitional probabilities algorithm will discover itself, application-free no need to train the agent in the target application -> train the agent like a real human being -> another big step towards general artificial intelligence.

%    \item Sandbox learning, can this be used to train DQN approach and applied on other games? This way one may not to deal with the context at all? reinforcement learning in games, may help other. Studying real-time/complex video games is hard due to lack SDK. Mainly, algorithms implemented on these game because they were the only available one. The need for the general AI trainer play ground Sukhbaatar et al. \cite{sukhbaatar2015mazebase}. Find the example that makes use of this (cannot find the paper atm. Starcraft AI player trained outside of the context i.e. by this sandbox).
\newpage
\nocite{*}
